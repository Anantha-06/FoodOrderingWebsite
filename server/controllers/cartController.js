...
export async function addToCart(req, res) {
  try {
    ...
    // 🔥 FIXED: Only check for conflict if items exist
    if (cart && cart.items.length > 0 && cart.restaurantId?.toString() !== restaurantId) {
    ...
    if (!cart) {
      ...
      cart.restaurantId = restaurantId; // Set restaurantId if cart was empty
    }
...
export async function addQuantity(req, res) {
  try {
    ...
    console.error("Unexpected error in addQuantity:", error);
